...
<span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">Anunciante</span>
</Link>
<div className="flex items-center gap-3">
<div className="text-sm font-medium tabular-nums">${(user?.balance || 0).toFixed(2)}</div>
<Button size="sm" onClick={() => setShowDeposit(true)} data-testid="advertiser-deposit-button"> <DollarSign className="w-4 h-4 mr-1" /> Depositar</Button>
...
{/* --- CAMPOS VOCAROO Y REFERENCIA --- */}
<div className="pt-4 border-t border-border/50">
<h3 className="text-sm font-semibold mb-3 flex items-center gap-2"><Megaphone className="w-4 h-4 text-teal-400"/> Multimedia e Instrucciones</h3>
<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
<div>
<Label className="text-teal-400">Enlace de Audio (Vocaroo) - Opcional</Label>
<input
type="text"
name="vocaroo_link"
...
className="w-full p-2 rounded bg-[hsl(var(--surface-2))] border border-border focus:border-teal-500 focus:outline-none text-sm mt-1"
/>
<p className="text-xs text-muted-foreground mt-1">Graba tu instrucción en vocaroo.com y pega el enlace.</p>
</div>
<div>
<Label className="text-blue-400">Enlace de Referencia (Doc, Web, etc.) - Opcional</Label>
<input
type="text"
name="reference_link"
placeholder="https://tu-marca.com/guia..."
value={campForm.reference_link || ""}
onChange={e => setCampForm({...campForm, reference_link: e.target.value})}
className="w-full p-2 rounded bg-[hsl(var(--surface-2))] border border-border focus:border-blue-500 focus:outline-none text-sm mt-1"
/>
</div>
</div>
</div>

<Button type="submit" className="w-full mt-4" disabled={campLoading}>{campLoading ? 'Creando...' : 'Crear Campana'}</Button>
</form>
</DialogContent>
</Dialog>
</div>
