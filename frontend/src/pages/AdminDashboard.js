                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                       <span className="text-xs bg-[hsl(var(--surface-3))] px-2 py-1 rounded font-mono">
                          ID Campaña: {c.campaign_unique_id}
                       </span>
                    </div>
...
// Helper component for Deliverables with Search
function DeliverablesList({ searchId }) {
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(true);

  const load = useCallback(() => {
    setLoading(true);
    deliverablesAPI.list().then(res => { setItems(res.data); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  ...
  const filtered = items.filter(item => {
    ...
    return item.campaign_unique_id?.includes(searchId) || item.campaign_id?.includes(searchId);
  });
                  ...
                  <div className="flex-1">
                    <div className="mb-2">
                       <span className="text-xs font-bold bg-primary/20 text-primary px-2 py-1 rounded">
                          ID Campaña: {item.campaign_unique_id || 'N/A'}
                       </span>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-medium text-sm">{item.creator_name}</span>
...
export default function AdminDashboard() {
  ...
  // State for searching deliverables by Campaign ID
        ...
        <Routes>
          ...
          <Route path="deliverables" element={
            <div>
              ...
              <DeliverablesList searchId={searchId} />
            </div>
          } />
...
// Generic Admin List Component (kept for other tabs)
function AdminList({ title, fetchFn, approveFn, rejectFn, columns, type }) {
  ...
  return (
    <div>
      ...
      <Input placeholder="Buscar..." value={filter} onChange={e => setFilter(e.target.value)} className="w-48 mb-4" />
      {loading ? (
        <div className="space-y-2">
          {[1,2,3].map(i => <div key={i} className="h-16 rounded-lg bg-[hsl(var(--surface-2))] animate-pulse" />)}
        </div>
