...
const map = { pending: 'status-badge-pending', approved: 'status-badge-approved', rejected: 'status-badge-rejected', active: 'status-badge-active', completed: 'status-badge-approved', cancelled: 'status-badge-rejected', accepted: 'status-badge-approved', verified: 'status-badge-approved', none: 'status-badge-pending' };
const labels = { pending: 'Pendiente', approved: 'Aprobado', rejected: 'Rechazado', active: 'Activo', completed: 'Completado', cancelled: 'Cancelado', accepted: 'Aceptado', verified: 'Verificado', none: 'Sin KYC' };
...
<span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">Creador</span>
</Link>
<div className="flex items-center gap-3">
{user?.is_top10 && <span className="text-xs bg-[hsl(43,96%,56%)]/20 text-[hsl(43,96%,56%)] px-2 py-0.5 rounded-full">Top 10</span>}
<div className="text-sm font-medium tabular-nums">${(user?.balance || 0).toFixed(2)}</div>
...
<div className="kpi-card"><p className="text-xs text-muted-foreground mb-1">Saldo</p><p className="text-2xl font-semibold tabular-nums text-primary" style={{fontFamily:'Space Grotesk'}}>${(user?.balance || 0).toFixed(2)}</p></div>
<div className="kpi-card"><p className="text-xs text-muted-foreground mb-1">Ganancias Totales</p><p className="text-2xl font-semibold tabular-nums" style={{fontFamily:'Space Grotesk'}}>${totalEarnings.toFixed(2)}</p></div>
<div className="kpi-card"><p className="text-xs text-muted-foreground mb-1">Aplicaciones Activas</p><p className="text-2xl font-semibold tabular-nums" style={{fontFamily:'Space Grotesk'}}>{acceptedApps.length}</p></div>
<div className="kpi-card"><p className="text-xs text-muted-foreground mb-1">Suscriptores</p><p className="text-2xl font-semibold tabular-nums" style={{fontFamily:'Space Grotesk'}}>{subscriptions.length}</p></div>
</div>
<div className="flex flex-wrap gap-3 mb-6">
<Button onClick={() => setShowProfile(true)} variant="outline"><Users className="w-4 h-4 mr-1"/> Editar Perfil</Button>
{user?.kyc_status !== 'verified' && <Button onClick={() => setShowKYC(true)} variant="outline"><ShieldCheck className="w-4 h-4 mr-1"/> Completar KYC</Button>}
...
{/* --- VOCAROO & REFERENCIA --- */}
{c.vocaroo_link && (
<div className="mt-3 p-3 bg-gray-800/50 rounded-lg border border-teal-500/30">
<p className="text-xs font-semibold mb-1 text-teal-400 flex items-center gap-1">🎧 Instrucciones de Voz</p>
<iframe
src={c.vocaroo_link.replace("vocaroo.com/", "vocaroo.com/embed/")}
width="100%" height="40" frameBorder="0" scrolling="no" allow="autoplay"
className="rounded"
></iframe>
</div>
)}
{c.reference_link && (
<div className="mt-2">
<p className="text-xs font-semibold mb-1 text-blue-400 flex items-center gap-1">🔗 Enlace de Referencia</p>
<a href={c.reference_link} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-300 underline hover:text-blue-200 break-all">
{c.reference_link}
</a>
</div>
)}
...
{c.bonus_amount > 0 && <p className="text-xs text-[hsl(43,96%,56%)] mt-1">Bonus: ${c.bonus_amount} si superas {c.bonus_threshold_views} vistas</p>}
</div>
{c.already_applied ? (
<span className="status-badge-active">Aplicado</span>
) : (
<Button size="sm" onClick={() => handleApply(c.id)} data-testid="campaign-apply-button"><ArrowRight className="w-4 h-4 mr-1"/> Aplicar</Button>
)}
</div>
</CardContent>
</Card>
...
<Card className="border-[hsl(var(--status-pending))]/30"><CardContent className="p-8 text-center"><p className="text-muted-foreground mb-3">No has completado tu perfil de creador</p><Button onClick={() => setShowProfile(true)}>Completar Ahora</Button></CardContent></Card>
)}
</TabsContent>
</Tabs>
</div>
...
<div><Label>Subir Archivo (opcional)</Label><Input type="file" onChange={e => setContentMedia(e.target.files[0])}/></div>
