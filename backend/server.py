...
async def register(req: RegisterReq):
    ...
    user = {
        ...
        "created_at": now_iso(), "creator_profile": None, "phone": "", "country": "",
        "gender": "", "referral_code": ref_code, "referred_by": referred_by,
        "profile_photo_url": "", "level_request_pending": False,
    }
...
async def update_settings(
    crypto_wallet_address: str = Form(""), crypto_network: str = Form("BEP20 (BSC)"),
    crypto_currency: str = Form("USDT/USDC"), bank_name: str = Form(""),
    bank_account_holder: str = Form(""), bank_account_number: str = Form(""),
    bank_details: str = Form(""), instructions: str = Form(""),
    user=Depends(require_admin)
):
    await db.platform_settings.update_one({}, {"$set": {
        "crypto_wallet_address": crypto_wallet_address, "crypto_network": crypto_network,
        "crypto_currency": crypto_currency, "bank_name": bank_name,
        "bank_account_holder": bank_account_holder, "bank_account_number": bank_account_number,
        "bank_details": bank_details, "instructions": instructions, "updated_at": now_iso(),
    }}, upsert=True)
    return {"message": "Configuración actualizada"}

# ===================== DEPOSITS =====================
@api.post("/deposits")
async def create_deposit(amount: float = Form(...), method: str = Form(...), reference: str = Form(""), proof: Optional[UploadFile] = File(None), user=Depends(get_current_user)):
    proof_url = ""
    if proof: proof_url = await save_upload(proof)
    deposit = {"user_id": user["sub"], "user_email": user["email"], "amount": amount, "method": method, "reference": reference, "proof_url": proof_url, "status": "pending", "created_at": now_iso(), "reviewed_by": None, "reviewed_at": None, "admin_note": ""}
...
async def approve_deposit(deposit_id: str, user=Depends(require_admin)):
    ...
    if not dep: raise HTTPException(404, "Depósito no encontrado o ya procesado")
    await db.deposits.update_one({"_id": ObjectId(deposit_id), "status": "pending"}, {"$set": {"status": "approved", "reviewed_by": user["sub"], "reviewed_at": now_iso()}})
    await db.users.update_one({"_id": ObjectId(dep["user_id"])}, {"$inc": {"balance": dep["amount"]}})
    await db.transactions.insert_one({"user_id": dep["user_id"], "type": "deposit", "amount": dep["amount"], "reference_id": str(dep["_id"]), "description": f"Depósito aprobado - {dep['method']} (${dep['amount']})", "created_at": now_iso()})
...
async def reject_deposit(deposit_id: str, note: str = "", user=Depends(require_admin)):
    result = await db.deposits.update_one({"_id": ObjectId(deposit_id), "status": "pending"}, {"$set": {"status": "rejected", "reviewed_by": user["sub"], "reviewed_at": now_iso(), "admin_note": note}})
    if result.modified_count == 0: raise HTTPException(404, "Depósito no encontrado o ya procesado")
...
async def create_campaign(req: CampaignCreate, user=Depends(require_advertiser)):
    ...
    await db.transactions.update_one({"user_id": user["sub"], "type": "campaign_escrow", "reference_id": ""}, {"$set": {"reference_id": str(result.inserted_id)}})
...
async def list_campaigns(status: str = Query(None), user=Depends(get_current_user)):
    query = {}
    if user["role"] == "advertiser": query["advertiser_id"] = user["sub"]
...
async def available_campaigns(user=Depends(require_creator)):
    u = await db.users.find_one({"_id": ObjectId(user["sub"])})
    query = {"status": "active"}
    if u.get("creator_profile"): cp = u["creator_profile"]
    campaigns = await db.campaigns.find(query).sort("created_at", -1).to_list(500)
...
async def upload_campaign_media(campaign_id: str, audio: Optional[UploadFile] = File(None), photo: Optional[UploadFile] = File(None), user=Depends(require_advertiser)):
    ...
    update = {}
    if audio: update["audio_url"] = await save_upload(audio)
    if photo: update["photo_url"] = await save_upload(photo)
    if update: await db.campaigns.update_one({"_id": ObjectId(campaign_id)}, {"$set": update})
...
async def cancel_campaign(campaign_id: str, user=Depends(get_current_user)):
    c = await db.campaigns.find_one({"_id": ObjectId(campaign_id)})
    if not c: raise HTTPException(404, "Campaña no encontrada")
    if user["role"] != "admin" and c["advertiser_id"] != user["sub"]: raise HTTPException(403, "No autorizado")
    if c["status"] != "active": raise HTTPException(400, "La campaña no está activa")
    ...
    if refund > 0:
        ...
        await db.transactions.insert_one({"user_id": c["advertiser_id"], "type": "campaign_refund", "amount": refund, "reference_id": campaign_id, "description": f"Reembolso por cancelación: {c['title']}", "created_at": now_iso()})
    return {"message": "Campaña cancelada", "refund": refund}

# ===================== APPLICATIONS =====================
@api.post("/applications")
async def create_application(campaign_id: str = Form(...), message: str = Form(""), user=Depends(require_creator)):
    existing = await db.applications.find_one({"campaign_id": campaign_id, "creator_id": user["sub"], "status": {"$in": ["pending", "accepted"]}})
    if existing: raise HTTPException(400, "Ya tienes una aplicación activa para esta campaña")
    recent_expired = await db.applications.find_one({"campaign_id": campaign_id, "creator_id": user["sub"], "status": {"$in": ["expired", "rejected"]}, "updated_at": {"$gte": (datetime.now(timezone.utc) - timedelta(hours=24)).isoformat()}})
    if recent_expired: raise HTTPException(400, "Debes esperar 24 horas antes de volver a aplicar a esta campaña")
    c = await db.campaigns.find_one({"_id": ObjectId(campaign_id), "status": "active"})
    if not c: raise HTTPException(404, "Campaña no encontrada o inactiva")
    max_per = c.get("max_videos_per_creator", 1)
    creator_deliverables = await db.deliverables.count_documents({"campaign_id": campaign_id, "creator_id": user["sub"], "status": "approved"})
    if creator_deliverables >= max_per: raise HTTPException(400, f"Has alcanzado el límite de {max_per} videos para esta campaña")
    u = await db.users.find_one({"_id": ObjectId(user["sub"])})
    app_doc = {"campaign_id": campaign_id, "campaign_title": c.get("title", ""), "creator_id": user["sub"], "creator_name": u.get("name", ""), "creator_email": u.get("email", ""), "message": message, "status": "pending", "created_at": now_iso(), "updated_at": now_iso(), "accepted_at": None, "deadline": None}
...
async def list_applications(campaign_id: str = Query(None), user=Depends(get_current_user)):
    query = {}
    if user["role"] == "creator": query["creator_id"] = user["sub"]
...
async def accept_application(app_id: str, user=Depends(get_current_user)):
    ...
    if not a: raise HTTPException(404, "Aplicación no encontrada")
    if user["role"] == "advertiser":
        c = await db.campaigns.find_one({"_id": ObjectId(a["campaign_id"])})
        if not c or c["advertiser_id"] != user["sub"]: raise HTTPException(403, "No autorizado")
    elif user["role"] != "admin": raise HTTPException(403, "No autorizado")
    deadline = (datetime.now(timezone.utc) + timedelta(hours=24)).isoformat()
    await db.applications.update_one({"_id": ObjectId(app_id)}, {"$set": {"status": "accepted", "accepted_at": now_iso(), "deadline": deadline, "updated_at": now_iso()}})
...
async def approve_deliverable(del_id: str, user=Depends(require_admin)):
    ...
    if not d: raise HTTPException(404, "Entregable no encontrado o ya procesado")
    ...
    if creator and creator.get("referred_by"):
        ...
        if referral_bonus > 0:
            ...
            await db.transactions.insert_one({"user_id": creator["referred_by"], "type": "referral_commission", "amount": referral_bonus, "reference_id": del_id, "description": f"Comisión de referido (5%): {d.get('creator_name','')}", "created_at": now_iso()})
...
async def reject_deliverable(del_id: str, note: str = "", user=Depends(require_admin)):
    result = await db.deliverables.update_one({"_id": ObjectId(del_id), "status": "pending"}, {"$set": {"status": "rejected", "reviewed_by": user["sub"], "reviewed_at": now_iso(), "admin_note": note}})
    if result.modified_count == 0: raise HTTPException(404, "Entregable no encontrado o ya procesado")
    return {"message": "Entregable rechazado"}

# ===================== KYC =====================
@api.post("/kyc")
async def submit_kyc(document: UploadFile = File(...), selfie: Optional[UploadFile] = File(None), user=Depends(get_current_user)):
    doc_url = await save_upload(document)
    selfie_url = ""
    if selfie: selfie_url = await save_upload(selfie)
    kyc = {"user_id": user["sub"], "user_email": user["email"], "document_url": doc_url, "selfie_url": selfie_url, "status": "pending", "created_at": now_iso(), "reviewed_by": None, "reviewed_at": None, "admin_note": ""}
...
async def approve_kyc(kyc_id: str, user=Depends(require_admin)):
    ...
    if not k: raise HTTPException(404, "KYC no encontrado o ya procesado")
...
async def request_withdrawal(req: WithdrawalReq, user=Depends(get_current_user)):
    u = await db.users.find_one({"_id": ObjectId(user["sub"])})
    if u["kyc_status"] != "verified": raise HTTPException(400, "Debes completar la verificación KYC antes de retirar fondos")
    if req.amount < 10: raise HTTPException(400, "El monto mínimo de retiro es $10")
    if u["balance"] < req.amount: raise HTTPException(400, f"Saldo insuficiente. Tienes ${u['balance']}")
    month_ago = (datetime.now(timezone.utc) - timedelta(days=30)).isoformat()
    recent = await db.withdrawals.count_documents({"user_id": user["sub"], "created_at": {"$gte": month_ago}, "status": {"$in": ["pending", "approved"]}})
    max_withdrawals = 3 if u.get("is_top10") else 1
    if recent >= max_withdrawals: raise HTTPException(400, f"Límite de retiros alcanzado ({max_withdrawals}/mes)")
    await db.users.update_one({"_id": ObjectId(user["sub"])}, {"$inc": {"balance": -req.amount}})
    withdrawal = {"user_id": user["sub"], "user_email": user["email"], "user_name": u.get("name", ""), "amount": req.amount, "method": req.method, "wallet_address": req.wallet_address, "bank_details": req.bank_details, "status": "pending", "created_at": now_iso(), "reviewed_by": None, "reviewed_at": None}
    result = await db.withdrawals.insert_one(withdrawal)
    await db.transactions.insert_one({"user_id": user["sub"], "type": "withdrawal_request", "amount": -req.amount, "reference_id": str(result.inserted_id), "description": f"Solicitud de retiro - {req.method}", "created_at": now_iso()})
...
async def approve_withdrawal(wd_id: str, user=Depends(require_admin)):
    ...
    if not w: raise HTTPException(404, "Retiro no encontrado o ya procesado")
...
async def reject_withdrawal(wd_id: str, user=Depends(require_admin)):
    ...
    await db.transactions.insert_one({"user_id": w["user_id"], "type": "withdrawal_refund", "amount": w["amount"], "reference_id": wd_id, "description": "Retiro rechazado - saldo devuelto", "created_at": now_iso()})
...
async def set_subscription_plan(req: SubPlanReq, user=Depends(require_creator)):
    await db.users.update_one({"_id": ObjectId(user["sub"])}, {"$set": {"subscription_plan": {"price": req.price, "description": req.description, "active": True}}})
...
async def subscribe_to_creator(creator_id: str = Form(...), user=Depends(get_current_user)):
    ...
    if not creator or not creator.get("subscription_plan", {}).get("active"): raise HTTPException(404, "Creador sin plan de suscripción activo")
    price = creator["subscription_plan"]["price"]
    fan = await db.users.find_one({"_id": ObjectId(user["sub"])})
    if fan["balance"] < price: raise HTTPException(400, f"Saldo insuficiente. Necesitas ${price}")
    existing = await db.subscriptions.find_one({"fan_id": user["sub"], "creator_id": creator_id, "active": True})
    if existing: raise HTTPException(400, "Ya estás suscrito a este creador")
    ...
    sub = {"fan_id": user["sub"], "creator_id": creator_id, "price": price, "active": True, "created_at": now_iso()}
    await db.subscriptions.insert_one(sub)
    await db.transactions.insert_one({"user_id": user["sub"], "type": "subscription_payment", "amount": -price, "reference_id": creator_id, "description": f"Suscripción a {creator.get('name','')}", "created_at": now_iso()})
    await db.transactions.insert_one({"user_id": creator_id, "type": "subscription_income", "amount": payout, "reference_id": user["sub"], "description": f"Ingreso por suscripción (comisión {int(rate*100)}%)", "created_at": now_iso()})
    await db.transactions.insert_one({"user_id": "__platform__", "type": "platform_commission", "amount": commission, "reference_id": creator_id, "description": f"Comisión suscripción {int(rate*100)}%", "created_at": now_iso()})
    return {"message": "Suscripción exitosa"}

@api.get("/subscriptions")
async def list_subscriptions(user=Depends(get_current_user)):
    if user["role"] == "creator": subs = await db.subscriptions.find({"creator_id": user["sub"], "active": True}).to_list(500)
    else: subs = await db.subscriptions.find({"fan_id": user["sub"], "active": True}).to_list(500)
    result = []
    for s in subs:
...
async def create_premium_content(content_type: str = Form("post"), title: str = Form(""), description: str = Form(""), media_url: str = Form(""), media: Optional[UploadFile] = File(None), user=Depends(require_creator)):
    media_file_url = ""
    if media: media_file_url = await save_upload(media)
    content = {"creator_id": user["sub"], "content_type": content_type, "title": title, "description": description, "media_url": media_url or media_file_url, "created_at": now_iso()}
...
async def get_premium_content(creator_id: str, user=Depends(get_current_user)):
    if user["sub"] != creator_id and user["role"] != "admin":
        ...
        if not sub: raise HTTPException(403, "Necesitas una suscripción para ver este contenido")
...
async def request_music_financing(title: str = Form(...), description: str = Form(""), amount_requested: float = Form(0), genre: str = Form(""), streaming_stats: str = Form(""), audio: Optional[UploadFile] = File(None), user=Depends(require_creator)):
    audio_url = ""
    if audio: audio_url = await save_upload(audio)
    req_doc = {"creator_id": user["sub"], "creator_email": user["email"], "title": title, "description": description, "amount_requested": amount_requested, "genre": genre, "streaming_stats": streaming_stats, "audio_url": audio_url, "status": "pending", "admin_message": "", "created_at": now_iso(), "reviewed_by": None, "reviewed_at": None}
...
async def approve_financing(req_id: str, message: str = "", amount: float = 0, user=Depends(require_admin)):
    ...
    await db.music_financing.update_one({"_id": ObjectId(req_id)}, {"$set": {"status": "approved", "admin_message": message, "amount_approved": final_amount, "reviewed_by": user["sub"], "reviewed_at": now_iso()}})
    await db.users.update_one({"_id": ObjectId(r["creator_id"])}, {"$inc": {"balance": final_amount}})
    await db.transactions.insert_one({"user_id": r["creator_id"], "type": "music_financing", "amount": final_amount, "reference_id": req_id, "description": f"Financiamiento musical: {r['title']}", "created_at": now_iso()})
...
async def reject_financing(req_id: str, message: str = "", user=Depends(require_admin)):
    result = await db.music_financing.update_one({"_id": ObjectId(req_id), "status": "pending"}, {"$set": {"status": "rejected", "admin_message": message, "reviewed_by": user["sub"], "reviewed_at": now_iso()}})
    if result.modified_count == 0: raise HTTPException(404, "Solicitud no encontrada o ya procesada")
...
async def get_rankings():
    ...
    if not rankings: return await generate_rankings()
...
async def generate_rankings():
    ...
    for c in creators:
        ...
        if not cp: continue
        entry = {"user_id": str(c["_id"]), "name": c.get("name", ""), "followers": cp.get("followers", 0), "avg_views": cp.get("avg_views", 0), "niche": cp.get("niche", ""), "region": cp.get("region", "")}
...
async def recalculate_rankings(user=Depends(require_admin)):
    ...
    if rankings:
        for r in rankings: await db.rankings.insert_one(r)
...
async def admin_change_role(user_id: str, role: str = Form(...), user=Depends(require_admin)):
    if role not in ["fan", "advertiser", "creator", "admin"]: raise HTTPException(400, "Rol inválido")
...
async def explore_creators(niche: str = Query(None), region: str = Query(None)):
    ...
    for c in creators:
        cp = c.get("creator_profile", {})
        if niche and cp.get("niche") != niche: continue
        if region and cp.get("region") != region: continue
...
async def get_creator_public(creator_id: str):
    ...
    if not c: raise HTTPException(404, "Creador no encontrado")
    return serialize_doc(c)
...
async def register_playlist(playlist_url: str = Form(...), playlist_name: str = Form(""), song_count: int = Form(...), followers: int = Form(0), user=Depends(require_creator)):
    if song_count < 5 or song_count > 25: raise HTTPException(400, "La playlist debe tener entre 5 y 25 canciones")
    if followers < 10: raise HTTPException(400, "La playlist debe tener al menos 10 seguidores/me gusta")
    playlist = {"user_id": user["sub"], "user_email": user["email"], "playlist_url": playlist_url, "playlist_name": playlist_name, "song_count": song_count, "followers": followers, "status": "active", "created_at": now_iso()}
...
async def curator_payment_request(playlist_id: str = Form(...), listens_count: int = Form(...), evidence_description: str = Form(""), evidence: Optional[UploadFile] = File(None), user=Depends(require_creator)):
    playlist = await db.curator_playlists.find_one({"_id": ObjectId(playlist_id), "user_id": user["sub"]})
    if not playlist: raise HTTPException(404, "Playlist no encontrada")
    evidence_url = ""
    if evidence: evidence_url = await save_upload(evidence)
    songs = playlist["song_count"]
    payout = 0.0
    for (s, l), rate in sorted(CURATOR_RATES.items(), key=lambda x: (-x[0][1], -x[0][0])):
        if songs >= s and listens_count >= l: payout = rate; break
    if payout == 0 and songs >= 5 and listens_count >= 100: payout = round(0.30 * (listens_count / 100) * (songs / 10), 2)
    request = {"user_id": user["sub"], "user_email": user["email"], "playlist_id": playlist_id, "playlist_name": playlist.get("playlist_name", ""), "song_count": songs, "listens_count": listens_count, "calculated_payout": payout, "evidence_url": evidence_url, "evidence_description": evidence_description, "status": "pending", "created_at": now_iso(), "reviewed_by": None}
...
async def approve_curator_request(req_id: str, user=Depends(require_admin)):
    ...
    await db.curator_requests.update_one({"_id": ObjectId(req_id)}, {"$set": {"status": "approved", "reviewed_by": user["sub"], "reviewed_at": now_iso()}})
    await db.users.update_one({"_id": ObjectId(r["user_id"])}, {"$inc": {"balance": payout}})
    await db.transactions.insert_one({"user_id": r["user_id"], "type": "curator_payment", "amount": payout, "reference_id": req_id, "description": f"Pago curador: {r['playlist_name']} ({r['listens_count']} escuchas)", "created_at": now_iso()})
...
async def reject_curator_request(req_id: str, note: str = "", user=Depends(require_admin)):
    result = await db.curator_requests.update_one({"_id": ObjectId(req_id), "status": "pending"}, {"$set": {"status": "rejected", "reviewed_by": user["sub"], "reviewed_at": now_iso(), "admin_note": note}})
    if result.modified_count == 0: raise HTTPException(404, "Solicitud no encontrada")
    return {"message": "Solicitud rechazada"}

# ===================== MICRO-TASKS =====================
@api.post("/micro-tasks")
async def submit_micro_task(songs_listened: int = Form(5), comment: str = Form(""), evidence: Optional[UploadFile] = File(None), user=Depends(get_current_user)):
    evidence_url = ""
    if evidence: evidence_url = await save_upload(evidence)
    payout = round(0.02 * (songs_listened / 5), 2)
    task = {"user_id": user["sub"], "user_email": user["email"], "songs_listened": songs_listened, "comment": comment, "evidence_url": evidence_url, "calculated_payout": payout, "status": "pending", "created_at": now_iso(), "reviewed_by": None}
...
async def approve_micro_task(task_id: str, user=Depends(require_admin)):
    ...
    if not t: raise HTTPException(404, "Tarea no encontrada o ya procesada")
    payout = t["calculated_payout"]
    await db.micro_tasks.update_one({"_id": ObjectId(task_id)}, {"$set": {"status": "approved", "reviewed_by": user["sub"], "reviewed_at": now_iso()}})
    await db.users.update_one({"_id": ObjectId(t["user_id"])}, {"$inc": {"balance": payout}})
    await db.transactions.insert_one({"user_id": t["user_id"], "type": "micro_task_payment", "amount": payout, "reference_id": task_id, "description": f"Micro-tarea: {t['songs_listened']} canciones escuchadas", "created_at": now_iso()})
...
async def reject_micro_task(task_id: str, user=Depends(require_admin)):
    result = await db.micro_tasks.update_one({"_id": ObjectId(task_id), "status": "pending"}, {"$set": {"status": "rejected", "reviewed_by": user["sub"], "reviewed_at": now_iso()}})
    if result.modified_count == 0: raise HTTPException(404, "Tarea no encontrada")
...
async def release_final_payment(del_id: str, user=Depends(require_admin)):
    ...
    if not d: raise HTTPException(404, "Entregable no encontrado o pago ya liberado")
    held = d.get("held_payout", 0)
    if held <= 0: raise HTTPException(400, "No hay pago pendiente")
    ...
    await db.transactions.insert_one({"user_id": d["creator_id"], "type": "campaign_payment_final", "amount": held, "reference_id": del_id, "description": f"Pago final 60% - permanencia cumplida", "created_at": now_iso()})
...
async def claim_bonus(del_id: str, user=Depends(get_current_user)):
    ...
    if not d: raise HTTPException(404, "Entregable no encontrado o bonus ya reclamado")
    if d["creator_id"] != user["sub"] and user["role"] != "admin": raise HTTPException(403, "No autorizado")
    c = await db.campaigns.find_one({"_id": ObjectId(d["campaign_id"])})
    bonus = c.get("bonus_amount", 0) if c else 0
    if bonus <= 0: raise HTTPException(400, "Esta campaña no tiene bonus")
    creator = await db.users.find_one({"_id": ObjectId(d["creator_id"])})
    commission_rate = 0.35 if creator and creator.get("is_top10") else 0.25
    ...
    await db.transactions.insert_one({"user_id": d["creator_id"], "type": "bonus_payment", "amount": bonus_payout, "reference_id": del_id, "description": f"Bonus reclamado (comisión {int(commission_rate*100)}%)", "created_at": now_iso()})
    await db.transactions.insert_one({"user_id": "__platform__", "type": "platform_commission", "amount": bonus_commission, "reference_id": del_id, "description": f"Comisión bonus {int(commission_rate*100)}%", "created_at": now_iso()})
    return {"message": f"Bonus de ${bonus_payout} reclamado"}

# ===================== LEVEL UP REQUEST =====================
@api.post("/creator/level-request")
async def request_level_up(requested_level: str = Form(...), justification: str = Form(""), portfolio_links: str = Form(""), user=Depends(require_creator)):
    u = await db.users.find_one({"_id": ObjectId(user["sub"])})
    if u.get("level_request_pending"): raise HTTPException(400, "Ya tienes una solicitud de nivel pendiente")
    req_doc = {"user_id": user["sub"], "user_email": user["email"], "user_name": u.get("name", ""), "current_level": u.get("creator_profile", {}).get("creator_level", "standard") if u.get("creator_profile") else "standard", "requested_level": requested_level, "justification": justification, "portfolio_links": portfolio_links, "status": "pending", "created_at": now_iso()}
...
async def approve_level_request(req_id: str, user=Depends(require_admin)):
    ...
    await db.users.update_one({"_id": ObjectId(r["user_id"])}, {"$set": {"level_request_pending": False, "creator_profile.creator_level": r["requested_level"]}})
...
async def create_ranking_board(name: str = Form(...), description: str = Form(""), user=Depends(require_admin)):
...
async def add_creator_to_board(board_id: str, creator_id: str = Form(...), user=Depends(require_admin)):
    creator = await db.users.find_one({"_id": ObjectId(creator_id)})
    if not creator: raise HTTPException(404, "Creador no encontrado")
...
async def admin_set_level(user_id: str, level: str = Form(...), user=Depends(require_admin)):
    await db.users.update_one({"_id": ObjectId(user_id)}, {"$set": {"creator_profile.creator_level": level}})
